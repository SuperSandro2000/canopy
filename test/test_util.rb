...
class CanubyTest < Minitest::Test
  ...
  Object.const_get(@@project).path = File.join(@@project.base_dir(true), @@project)
  ...
  def test_paths
    assert_equal('testing', Paths.base_dir(true))
    assert_equal(File.join(base_dir(true), 'Test', 'build'), Paths.build_dir(@@project))
    assert_equal(File.join(base_dir(true), 'lib'), Paths.stage_dir(true))
  ...
  def test_git
    Git.clone(@@project)
    assert_exists('testing/test/CMakeLists.txt')
