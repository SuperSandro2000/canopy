...
module Paths
  ...
  def build_dir(project)
    File.join(Object.const_get(project).path, 'build')
  end

  def stage_dir(test = false)
...
module Outputs
  def build(project)
    ...
    if output_dir
      const_get(project).outputs.map { |f| File.join(const_get(project).build_dir(project), output_dir, ENV['reL_type'], f) }
    else
      const_get(project).outputs.map { |f| File.join(const_get(project).build_dir(project), ENV['reL_type'], f) }
    end
  end

  def stage(project, test = false)
    const_get(project).outputs.map { |f| File.join(stage_dir(test), f) }
  end
end
...
module Git
  def clone(project)
    ...
    system("git clone #{const_get(project).url} #{const_get(project).path.downcase}")
  end
...
module Build
  def msbuild(project, project_file)
...
class Project
  ...
  attr_accessor :url, :version, :path, :project_file, :output_dir, :outputs
end
